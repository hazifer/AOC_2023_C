...
int get_points_from_line(char *, int *, int *, int, int);
...
{
	char line[1000];
	int points = 0, guess_cards_num, winning_cards_num;
	...
	points += get_points_from_line(line, guess_cards, winning_cards, guess_cards_num, winning_cards_num);
	while (fgets(line, 1000, fp))
		points += get_points_from_line(line, guess_cards, winning_cards, guess_cards_num, winning_cards_num);
	printf("guess: %d winning: %d\n", guess_cards_num, winning_cards_num);
	printf("total points: %d\n", points);
	return 0;
}

int get_points_from_line(char *line, int *guess_cards, int *winning_cards, int guess_cards_len, int winning_cards_len)
{
	int points, similar_count;
	char *tmp;
	line = strchr(line, ':');
	if (!line)
		return 0;
